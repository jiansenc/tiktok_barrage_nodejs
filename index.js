...
const chatObserverrom = new MutationObserver((mutationsList, observer) => {
	for (let mutation of mutationsList) {
		if (mutation.type === 'childList' && mutation.addedNodes.length) {
			let dom = mutation.addedNodes[0]
			// console.log(dom[propsId].children.props.children.props)
			let { payload } = dom[propsId].children.props.children.props.message
...
function setDio() {
	setInterval(() => {
		let dio = document.querySelector('.summon-iframe-contrainer')
		if (dio) {
			let btn = document.querySelector('.OG51D9OO')
			if (btn) {
				btn.click()
			}
		}
